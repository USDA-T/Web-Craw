...
public class TestAnalystEDWOSBReviewWorkflow extends TestCase {
    ...
    public void testMainTest() throws Exception {
        try {
            ...
            Thread.sleep(2000);
            logger_TestEDWOSBWorkflow.info(webDriver.findElement(By.xpath("//p[ (b[contains(text(),'DUNS:')]) ]"))
                    .getText().replaceAll("DUNS:", "").trim());
            String duns_Number = webDriver.findElement(By.xpath("//p[ (b[contains(text(),'DUNS:')]) ]")).getText()
                    .replaceAll("DUNS:", "").trim();
            String typ_App = "EDWOSB";
            // Clear EDWOSB Application
            if (commonApplicationMethods.checkApplicationExists(webDriver, "EDWOSB", "Active")) {
                commonApplicationMethods.returnApplicationToVendorMethd(webDriver, 11, duns_Number, "EDWOSB", "Active",
