...
public class TestUs1187MppPrepareLinkAndCopyUdates extends TestCase {
    ...
    private static final Logger logger = LogManager.getLogger(TestUs1187MppPrepareLinkAndCopyUdates.class);
    ...
    public void testMainTest() throws Exception {
        ...
        try {
            ...
            List<WebElement> all_link = webDriver
                    .findElement(By.xpath("//div[contains(@class,'usa-width-one-whole') and contains(@class,'usa-content')]"))
            ...
            logger.info(webDriver.findElement(By.xpath("//div [  ul [ contains(@class, 'ul-level-one' ) ]  ] ")).getText());
            logger.info(webDriver.findElement(By.xpath("//div/ul [ contains(@class, 'ul-level-one' ) ]   ")).getText());
            logger.info(Actual_Text);
            // Below to Assert the new Mpp Text added
            Actual_Text = webDriver.findElement(By.xpath("//div/p[contains(text(),'ust meet the size standard for small in the NAICS code')]")).getText();
            Expected_Text = "A Mentor-Protégé relationship should be established before starting the application – this is not a matching program. The Protégé firm must meet the size standard for small in the NAICS code in which they are seeking business development assistance.";
