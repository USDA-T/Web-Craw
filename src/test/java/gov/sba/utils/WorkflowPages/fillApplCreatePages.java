...
public class fillApplCreatePages {
	...
	public static void pageCaseOverviewFillup(WebDriver webDriver, String review_Type, String curr_Reviewer, String owner, String supervisor) throws Exception {

		try {
			//			webDriver.findElement(By.xpath("//*[@id='question-review']//a[contains(text(),'Case overview')]")).click();
			webDriver.findElement(By.xpath("//ul[@class='usa-sidenav-list']/li/a[contains(text(),'ase overview')]")).click();
			...
			if (owner.length() > 0) {
				Select dropdown1 = new Select(webDriver.findElement(By.xpath("//select[@id='review_current_assignment_attributes_owner_id']")));
				dropdown1.selectByVisibleText(owner);
			}

			if (supervisor.length() > 0) {
				Select dropdown1 = new Select(webDriver.findElement(By.xpath("//select[@id='review_current_assignment_attributes_supervisor_id']")));
