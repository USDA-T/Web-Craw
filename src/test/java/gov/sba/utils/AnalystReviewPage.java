...
public class AnalystReviewPage extends TestCase {
    ...
    public void TestReviewDriver(WebDriver mydriver, String duns_Numb, String typ_App, String rev_Type_p, String curr_Review_p, String owner_p, String supervisor_p) {
    ...
    public void testSubmitted() throws Exception {
        try {
            ...
            //assertEquals("Currently No applications =" ," in Submitted state");
            ...
            current_Row = webDriver.findElements(
                    By.xpath("//div[@id='table-search']/table/tbody/tr[ " + "td[position()=8 and contains(text(),'Submitted')]   and " + "td[position()=2 and contains(text(),'" + duns_Number + "')]	and " + "td[position()=3 and contains(text()," + typ_App_Passed + ")]	" + "]"));

            AnalystReviewPage.info(current_Row.size() + ": Is the total Submitted Elements");
            if (current_Row.size() <= 0) {
                //assertEquals("Currently No applications =" ," Could be submitted");
            ...
            WebElement current_Page_Title = webDriver.findElement(By.xpath("//article[@id='main-content']/div/div[2]/h1"));
            ...
            String sql_query = " select D.workflow_state as review_status,D.sba_application_id as app_id , D.updated_at as updated_at" + "   from sbaone.organizations A, sbaone.certificates B, sbaone.sba_applications C, sbaone.reviews D " + "    where A.id = B.organization_id  " + "    and B.sba_application_id = C.id " + "    and c.id = D.sba_application_id " + "    and b.id = D.certificate_id     " + "    and  A.duns_number='159165917' " + "    order by D.updated_at desc ";

            DatabaseQuery dbcall = new DatabaseQuery();
            String[][] returned_workflow_review_status = dbcall.getDBData(sql_query, 2, 3);
            AnalystReviewPage.info("aaaaa+" + returned_workflow_review_status[1][0].toString() + returned_workflow_review_status[1][1].toString() + returned_workflow_review_status[1][2].toString());
            Thread.sleep(1000);
            assertEquals(returned_workflow_review_status[1][0].toString().toLowerCase(), "cancelled");

            //verify the review status as cancelled- start again review assignment
    ...
    public void testUnderReview() throws Exception {
        try {
            ...
            webDriver.findElement(By.xpath("//ul[@class='usa-sidenav-list']/li/a[contains(@href,'signature_review')]")).click();
