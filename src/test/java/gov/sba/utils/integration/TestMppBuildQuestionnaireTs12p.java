...
public class TestMppBuildQuestionnaireTs12p extends TestCase {
	...
	public void testMppBuildQuestionnaireTs12p() throws Exception {
		...
		jse.executeScript("arguments[0].scrollIntoView()", webDriver.findElement(By.linkText("13 CFR 124.520(d)(2)")));
		...
		wait.until(ExpectedConditions
				.elementSelectionStateToBe(By.xpath("//div[@id='answers_mpp_completion_cert']/fieldset/h4"), false));
		...
		Actual_Text = webDriver.findElement(By.xpath("//div[@id='answers_other_active_mpp_agreements']/fieldset/p[2]"))
				.getText();
		Expected_Text = "To participate in the All Small Mentor-Protégé Program, a protégé may have no more than two mentors across SBA’s Mentor-Protégé programs. Mentors may have no more than three protégés. Reference: 13 CFR 125.9";
		assertEquals(Actual_Text, Expected_Text);
		// Click on the continue button without answering the question and
		// verify alert message.
		CoreUtils.clickContinue(webDriver);
		...
		logger.info("Before switching, title is = certify.sba.gov");
		jse.executeScript("arguments[0].scrollIntoView()",
				webDriver.findElement(By.linkText("completing a business plan")));
		...
		logger.info("Before switching, title is = certify.sba.gov");
		jse.executeScript("arguments[0].scrollIntoView()",
				webDriver.findElement(By.linkText("local resource partner near you.")));
		...
		act.doubleClick(webDriver.findElement(By.xpath("//fieldset/a/span"))).build().perform();
		// Select Role.
		...
		// Remove agreement.
		webDriver.findElement(By.xpath("//div[2]/div/h6/a")).click();
		...
		assertEquals(Actual_Text, Expected_Text);
		WebElement ActiveA = webDriver.findElement(By.xpath("//article/h2"));
		...
		// are being saved.
		// Financial needs.
		jse.executeScript("arguments[0].scrollIntoView()", webDriver.findElement(By.id("management_and_tech")));
		...
		jse.executeScript("arguments[0].scrollIntoView()", webDriver.findElement(By.id("financial")));
		...
		jse.executeScript("arguments[0].scrollIntoView()", webDriver.findElement(By.xpath("//td[3]")));
