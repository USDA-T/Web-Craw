 ...
 * annual report Mpp analyst gets Submitted the MPP annual report  ..
 */
...
public class TestWorkflowMPPReportSubmit extends TestCase {
    ...
    public void testWorkflowMPPReportSubmit() throws Exception {
      ...
      /* Verify on the Vendor dashbaord page -App-1279*/
          ...
          assertNotNull(verify_Row_In_A_Table_And_Return(webDriver, new String[] {"MPP Application","Annual Report","","","Active","","View summary"}));

      /* For Demo on -7-21 */

        } catch (Exception e) {
            logger.info("Search TextBox is on Main Navigator is not present" + e.toString());
            take_ScreenShot_TestCaseName(webDriver, new String[] {"TestWorkflowMPPReportSubmit", "Exception"});
            throw e;
