  ...
  public void testMainTest() throws Exception {
    try {
      ...
      try {
        ...
        return_All_Applications(webDriver, 55, "144754156");
        delete_All_Application_Draft(webDriver, get_The_Row_From_Login_Data, duns_Number);
        new LoginPageWithReference(webDriver, 42).Login_With_Reference();
        /* Verify the Business type is changes to LLC] */
        click_Element(webDriver, "Vendor_Admin_Dashboard_More_Details");
        assertTrue(find_Element(webDriver, "Vendor_Admin_Dashboard_More_Details_Id").getText().contains("Limited Liability"));
        join_New_Program_CheckBoxes(webDriver, "WOSB");
      ...
      try {
        ...
        TestuserProfileSearchType.TestuserProfileSearch(webDriver, returned_GovProfile_Rows[1][1], "OppSup_Dashboard_Govt_User_Radio_Bt", Expected_Result);
        webDriver.navigate().back();

        sql_query = "select email,first_name,last_name from sbaone.users where max_id is null and email like 'QA%'";
        ...
        TestuserProfileSearchType.TestuserProfileSearch(webDriver, areturned_Rows[1][1], "OppSup_Dashboard_Vend_User_Radio_Bt", Expected_Result1);
        webDriver.navigate().back();
      } catch (Exception e) {
        logger.info(e.toString());
      }
      ...
      take_ScreenShot_TestCaseName(webDriver, new String[] {"TestWorkflowAppOppSupportAdmin", "Exception"});
      throw e;
    }
