...
public class TestWOSBRenewal extends TestCase {
    ...
    public void testMainTest() throws Exception {
        ...
        try {
            ...
            // expire
            // --after the midnight cron job runs check the status of the
            // certificate to - Expired -- APP387
            String sql_Q_01 = "update sbaone.certificates set expiry_date = CURRENT_TIMESTAMP where organization_id = (select id from sbaone.organizations where duns_number = '"
                    + duns_Number + "')";
            new DatabaseUtils().executeSQLScript(sql_Q_01);
            // check the status of the certificate to - Expired - verify the
            // Renewal link - submit new renew application
            ...
            page8aFillUp(webDriver, "Yes", FixtureUtils.fixturesDir() + "Upload.pdf");
            finalSignatureSubmit(webDriver);
            // Verify the old application's status - Expired The renewed
            // Application's status - Active
            ...
            List<WebElement> count_Edwosb_Expire = find_Elements(webDriver, "xpath",
                    "//*[@id='certifications']/tbody/tr" + "[" + "td[position()=1]/a[contains(text(),'EDWOSB')]"
                            + " and " + "td[position()=5 and (contains(text(),'Expired'))]" + "]");
