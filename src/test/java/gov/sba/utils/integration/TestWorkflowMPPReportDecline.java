 ...
 * TestWorkflowMPPReportDecline Vendor admin create MPP application ,approved by supervisor submit Mpp
 * annual report. Mpp analyst review the MPP annual report and Declined report.
  */
...
public class TestWorkflowMPPReportDecline extends TestCase {
    Logger logger =
        LogManager.getLogger(gov.sba.utils.integration.TestWorkflowMPPReportDecline.class.getName());
    ...
    @Test
    public void testWorkflowMPPReportDecline() throws Exception {
        try {
      /*App-!279,App-1219
      ...
      /* Submitted Mpp Annual report below assertions*/
            ...
            click_Element(webDriver,"SBA_MPP_Report_Decline");
      ...
      /* Verify on teh Vendor dashbaord page -App-1279*/
            ...
            assertNotNull(verify_Row_In_A_Table_And_Return(webDriver, new String[] {"MPP Application","Annual Report","","","Active","Declined","View summary"}));

      /* For Demo on -7-21 */

        } catch (Exception e) {
            logger.info("Search TextBox is on Main Navigator is not present" + e.toString());
            take_ScreenShot_TestCaseName(webDriver, new String[] {"TestWorkflowMPPReportDecline", "Exception"});
            throw e;
