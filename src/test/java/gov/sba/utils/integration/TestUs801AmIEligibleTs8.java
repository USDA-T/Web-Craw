...
public class TestUs801AmIEligibleTs8 extends TestCase {
    ...
    try {
        ...
        wait.until(ExpectedConditions.elementSelectionStateToBe(By.xpath("//div[@id='eighta_certified']/div/div/p"),
                false));
    ...
    if (webDriver.getCurrentUrl().contains("newqa.sba")) {
      try {
       ...
       logger.info("Success No NAICS in local ENV");
        ...
        logger.info("Please make sure the env you are running the test is added to the params, fyi env is " +webDriver.getCurrentUrl());
        ScreenShotPage screenShot = new ScreenShotPage(webDriver);
        screenShot.ScreenShot();
        Assert.fail();
      }
