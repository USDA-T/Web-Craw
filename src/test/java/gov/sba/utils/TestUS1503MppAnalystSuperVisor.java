...
public class TestUS1503MppAnalystSuperVisor extends TestCase {
	...
	public void testMainTest() throws Exception {
		...
		// Log in As Analyst/Supervisor - validate as per the US1503 Acceptance criteria
		...
		try {
			...
			ResultSet result_Set = statement_SQL.executeQuery(" SELECT F.legal_business_name AS legal_Name, "
					...
					+ "G.id as certificate_ID ,P.id as group_id " 	
					+"	FROM "
			...
			List<WebElement> current_Row_Not_MPP = webDriver.findElements(By.xpath("//div[@id='table-search']/table[contains(@class,'usa-table')]/tbody/tr/td[position()=3 and not(contains(text(),'MPP')) ]"));
			...
			WebElement current_Row_MPP = webDriver.findElement(By.xpath("//div[@id='table-search']/table[contains(@class,'usa-table')]/tbody/tr/td[text()='MPP']"));
