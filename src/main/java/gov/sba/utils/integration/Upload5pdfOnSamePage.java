	...
	public void Upload5pdfOnSame(String upload_Path) throws Exception {
		...
		if (webDriver.getCurrentUrl().contains("certify.qa")) {
			...
			WebElement rateElement2 = webDriver
					.findElement(By.xpath("(//a[contains(text(),'Add required documents')])[5]"));
			...
			WebElement rateElement1 = webDriver.findElement(By.xpath("(//button[@id='doc-lib-button'])[5]"));
			((JavascriptExecutor) webDriver).executeScript("arguments[0].click();", rateElement1);
			wait.until(ExpectedConditions.elementToBeClickable(By.xpath("(//td[@id='document_library_file_name']/ul/li/label)[5]")));
			WebElement rateElement11 = webDriver.findElement(By.xpath("(//td[@id='document_library_file_name']/ul/li/label)[5]"));
			((JavascriptExecutor) webDriver).executeScript("arguments[0].click();", rateElement11);	
			WebElement rateElement111 = webDriver
					.findElement(By.xpath("(//div[@id='document_library']/fieldset/div/div/button)[5]"));
			...
			if (webDriver.getCurrentUrl().contains("localhost")) {
				...
				if (webDriver.getCurrentUrl().contains("newqa")) {
					...
					WebElement rateElement1 = webDriver.findElement(By.xpath("(//button[@id='doc-lib-button'])[5]"));
					((JavascriptExecutor) webDriver).executeScript("arguments[0].click();", rateElement1);
					WebElement rateElement11 = webDriver.findElement(By.xpath("(//input[@id='truth'])[5]"));
					((JavascriptExecutor) webDriver).executeScript("arguments[0].click();", rateElement11);
					WebElement rateElement111 = webDriver
							.findElement(By.xpath("(//div[@id='document_library']/fieldset/div/div/button)[5]"));
					((JavascriptExecutor) webDriver).executeScript("arguments[0].click();", rateElement111);
					...
					act.doubleClick(webDriver.findElement(By.xpath("(//a[contains(text(),'Choose a .pdf file')])[5]")))
							.build().perform();
