...
public class AddOrStartCertificationPage {
	...
	public void AddOrStartCertification() throws Exception {
		...
		if (webDriver.getPageSource().contains("Active")) {
			...
			try {
				Actual_Text = webDriver.findElement(By.xpath("//article/div/div")).getText();
				...
				assertEquals(Actual_Text, Expected_Text);
				Actual_Text = webDriver.findElement(By.xpath("//article/div/div[2]")).getText();
				...
				WebElement rateElement71 = webDriver.findElement(By.linkText("EDWOSB self-certification"));
