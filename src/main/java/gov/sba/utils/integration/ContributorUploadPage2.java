...
public class ContributorUploadPage2 {
	private static final Logger logger = LogManager.getLogger(ContributorUploadPage2.class.getName());
	WebDriver webDriver;

	public ContributorUploadPage2(WebDriver webDriver) {
	...
	public void ContributorUpload(String upload_Path) throws Exception {
		...
		act.doubleClick(webDriver.findElement(By.xpath("(//a[contains(text(),'Choose a .pdf file')])[2]"))).build()
				.perform();
		...
		jse.executeScript("arguments[0].scrollIntoView()",
				webDriver.findElement(By.xpath("(//button[@id='attach'])[2]")));
