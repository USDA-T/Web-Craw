...
public class AnalystReviewPage extends TestCase {
    ...
    public void testUnderReview() throws Exception {
        try {
            String xpath = "//div[@id='table-search']/table/tbody/tr[ td[position()=2]/a[contains(text(),'"
            ...
            if (current_Row.size() > 0) {
                ...
                Assert.assertEquals(find_Element(webDriver, "SBA_Question_New_Determination_Review_Started").getText(),
                        "Review Started");
                Assert.assertEquals(find_Element(webDriver, "SBA_Question_New_Determination_Return_For_Mod").getText(),
                        "Return for Modification");
                Assert.assertEquals(
                        find_Element(webDriver, "SBA_Question_New_Determination_Reccomend_For_Eligibile").getText(),
                        "Recommend Eligible");
                Assert.assertEquals(
                        find_Element(webDriver, "SBA_Question_New_Determination_Reccomend_For_InEligibile").getText(),
