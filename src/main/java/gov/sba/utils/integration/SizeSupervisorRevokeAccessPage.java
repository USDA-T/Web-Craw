...
public class SizeSupervisorRevokeAccessPage extends TestCase {

	private static final Logger logger = LogManager.getLogger(SizeSupervisorRevokeAccessPage.class.getName());
	WebDriver webDriver;
	int get_The_Row_From_Login_Data;

	public SizeSupervisorRevokeAccessPage(WebDriver webDriver) {
		this.webDriver = webDriver;
		get_The_Row_From_Login_Data = 101;

	}

	public void SizeSupervisorRevokeAccess() throws Exception {
		WebDriverWait wait = new WebDriverWait(webDriver, 30);
		String Actual_Text = null;
		String Expected_Text = null;
		logger.info("Completing Process, Size supervisors Revoked Access Request from another size users.");
		// Login to dashboard.
		...
		try {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//body/section/article/h1")));
