...
public class BasicEligibilityMasterAppPage extends TestCase {
  private static final Logger logger =
      LogManager.getLogger(BasicEligibilityMasterAppPage.class.getName());
  WebDriver webDriver;
  ...
  public void BasicEligibilityMasterApp() throws Exception {
    ...
    Actual_Text = webDriver.findElement(By.cssSelector("h4")).getText();
    Expected_Text =
        "Please upload the size determination or redetermination letter(s) issued by SBA.";
    assertEquals(Actual_Text, Expected_Text);
    // Upload Doc.
    file_path_abs = FixtureUtils.fixturesDir() + "MainTestUploadDoc.pdf";
    ...
    Actual_Text = webDriver
        .findElement(By.xpath("//div[@id='answers_sba_area_office']/fieldset/div/div[2]/select"))
        .getText();
    Expected_Text =
        "SBA Area Office 1\nSBA Area Office 2\nSBA Area Office 3\nSBA Area Office 4\nSBA Area Office 5\nSBA Area Office 6\nSBA Headquarters";
    assertEquals(Actual_Text, Expected_Text);
    ...
    assertEquals(Actual_Text, Expected_Text);
    // Click on the change answer for Outside Assistance and change it from yes to no.
    ...
    CoreUtils.clickContinue(webDriver);
    // Verify status.
    wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//td[3]")));
    ...
    webDriver.findElement(By.xpath("//div/input")).click();
    wait.until(ExpectedConditions
        .visibilityOfElementLocated(By.xpath("//div[@id='disqualifier-warning']/div/div/p")));
    Actual_Text =
        webDriver.findElement(By.xpath("//div[@id='disqualifier-warning']/div/div/p")).getText();
