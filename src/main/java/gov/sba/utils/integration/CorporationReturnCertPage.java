...
public class CorporationReturnCertPage extends TestCase {
	private static final Logger logger = LogManager.getLogger(CorporationReturnCertPage.class.getName());
	WebDriver webDriver;
	int get_The_Row_From_Login_Data;

	public CorporationReturnCertPage(WebDriver webDriver) {
		this.webDriver = webDriver;
	}

	public void CorporationReturnCert() throws Exception {
		...
		Expected_Text = "By submitting this certification I, Deric Vendor, am an officer or owner of Entity 127 Legal Business Name authorized to represent it and electronically sign this certification on its behalf.";
		assertEquals(Actual_Text, Expected_Text);
		...
		if (webDriver.getCurrentUrl().contains("certify.qa")) {
			...
			if (webDriver.getCurrentUrl().contains("staging")) {
				...
				webDriver.findElement(By.xpath("//form/div/button")).click();
				wait.until(
						ExpectedConditions.visibilityOfElementLocated(By.linkText("Entity 127 Legal Business Name")));
				webDriver.findElement(By.linkText("Entity 127 Legal Business Name")).click();
			} else {
				if (webDriver.getCurrentUrl().contains("newqa")) {
					...
					wait.until(ExpectedConditions
							.visibilityOfElementLocated(By.linkText("Entity 127 Legal Business Name")));
					webDriver.findElement(By.linkText("Entity 127 Legal Business Name")).click();
				} else {
