...
public class ContributorOtherIndividualsQuestionPage extends TestCase {
  private static final Logger logger =
      LogManager.getLogger(ContributorOtherIndividualsQuestionPage.class.getName());
  WebDriver webDriver;
  ...
  public void ContributorOtherIndividualsQuestion() throws Exception {
    try {
      ...
      wait.until(ExpectedConditions.elementSelectionStateToBe(By.linkText("Add other individuals"),
          false));
      ...
      webDriver.findElement(By.linkText("Add other individuals")).click();
      wait.until(ExpectedConditions
          .visibilityOfElementLocated(By.xpath("//form[3]/fieldset/ul/li/label")));
      // Validate the send invitation button is present and to make sure users can't send empty data
      ...
      // Verify intro page for 8(a) Business Partner(other individuals).
      ...
      // CoreUtils.clickContinue(webDriver);
      // Actual_Text = webDriver.findElement(By.cssSelector("error")).getText();
      // Expected_Text = "The start date should come before the end date";
      // assertEquals(Actual_Text, Expected_Text);
      // } catch (Error e) {
      // logger.info(e.getMessage());
      // }
